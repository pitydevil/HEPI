...
class ModalCheckInOutViewController: UIViewController {
    ...
    private var checkInDateObject  = BehaviorRelay<Date>(value:Date())
    private var checkOutDateObject = BehaviorRelay<Date>(value:Date())
    private var checkFinalObject = BehaviorRelay<CheckIn>(value:CheckIn(checkInDate: Date(), checkOutDate: Date()))
    
    var checkInObjectObserver: Observable<Date> {
    ...
    private lazy var headline: ReuseableLabel = {
        let headline = ReuseableLabel(labelText: "Start Date", labelType: .titleH1, labelColor: .black)
        return headline
    }()
    
    private lazy var headlineSecond: ReuseableLabel = {
        let headline = ReuseableLabel(labelText: "End Date", labelType: .titleH1, labelColor: .black)
        return headline
    }()
...
extension ModalCheckInOutViewController: UICalendarViewDelegate, UICalendarSelectionSingleDateDelegate {
    ...
    func dateSelection(_ selection: UICalendarSelectionSingleDate, didSelectDate dateComponents: DateComponents?) {
        ...
        checkInDateObject.accept((dateComponents?.date)!)
        checkOutDateObject.accept((calendarViewSecond.date))
    }
    ...
    @objc func valueChanged(_ sender: UIDatePicker) {
        checkOutDateObject.accept(sender.date)
    }
}
